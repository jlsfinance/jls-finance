  ...
  const handleUserUpdate = async () => {
    ...
    try {
      ...
      if (editedRole === 'agent') {
        ...
        // Remove permissions if not an agent
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Manage user roles and agent-specific permissions.</CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
                    ...
                    <TableCell className="text-right">
                      <Dialog onOpenChange={(isOpen) => !isOpen && setSelectedUser(null)}>
                        <DialogTrigger asChild>
                          <Button variant="outline" size="sm" onClick={() => openEditDialog(user)}><Edit className="mr-2 h-4 w-4" /> Edit User</Button>
                        </DialogTrigger>
                        {selectedUser && selectedUser.id === user.id && (
                          <DialogContent className="sm:max-w-[480px]">
                            <DialogHeader>
                              <DialogTitle>Edit User: {selectedUser.name}</DialogTitle>
                              <DialogDescription>Modify the user's role and permissions. Changes are applied immediately.</DialogDescription>
                            </DialogHeader>
